    ...
    return (
        <Box p='5' maxW='3lg' mx='auto'>
                ...
                <Table px='3' border='2px solid' borderColor='gray.100'>
                    <Thead backgroundColor='gray.100'>
                        <Tr>
                            ...
                            <Th>
                                <Flex
                                    onClick={() => handleOrderBy("spentTime")}
                                    cursor="pointer"
                                    alignItems="center"
                                >
                                    Time Spent {searchParams.get("orderBy") === "spentTime" && <BsArrowUp />}
                                </Flex>
                            </Th>
                        ...
                        {tasks.map(task => (
                            <Tr key={task._id}>
                                ...
                                <Td>{task.due ? new Date(task.due).toDateString() :
                                    ''}</Td>
                                <Td>{task.spentTime ? `${task.spentTime} hrs` : "Не записано"}</Td>
            ...
            <Heading as="h3" size="md">
                Общее кол-во времени, затраченное на выполнение задач:{" "}
                {tasks.reduce((total, task) => total + (task.spentTime || 0), 0)} hrs
            </Heading>
        </Box>
